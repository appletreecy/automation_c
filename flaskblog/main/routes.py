...
def apt2():
    ...
    time.sleep(1)
    launchBpsAttack("Victor-Virus-Web-Site")  # Changed
    time.sleep(1)
...
def apt5():
    form = PostForm()
    launchBpsAttack("Victor-Windows-RDP-Attack")  # Changed
    time.sleep(1)
...
def netflow_demo():
    ...
    username = 'root'
    password = 'toor'
    cmd = '/root/netflow_cnc_script.sh'
...
# The First Netflow Attack in Netflow Lab Guide Page


@main.route("/attack/netflow_cnc", methods=['GET', 'POST'])
@login_required
def netflow_cnc():
    form = PostForm()
    ip = '172.16.14.38'
    ...
    return render_template('netflow_labguide.html', title='test', form=form)

# The Second Netflow Attack in Netflow Lab Guide Page


@main.route("/attack/netflow_scan", methods=['GET', 'POST'])
@login_required
def netflow_scan():
    form = PostForm()
    ip = '172.16.14.38'
    ...
    username = 'root'
    password = 'toor'
    cmd = '/root/netflow_scan_script.sh'
...
@main.route("/netflow_labguide")
def netflow_labguide():
    return render_template('netflow_labguide.html', title='About')


@main.route("/firewall_labguide")
def firewall_labguide():
    return render_template('firewall_labguide.html', title='About')
...
@main.route("/attack/firewall_syn_flood", methods=['GET', 'POST'])
@login_required
...
@main.route("/attack/firewall_ddos", methods=['GET', 'POST'])
@login_required
def firewall_ddos():
    form = PostForm()
...
@main.route("/attack/firewall_packetcapture", methods=['GET', 'POST'])
@login_required
